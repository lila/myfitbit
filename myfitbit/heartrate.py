...
def getHeartrateAsDataframe(f, dates):
    first = True
    for day in dates:
        fit_statsHR = f.intraday_time_series('activities/heart', base_date=day, detail_level='1sec')
        if not fit_statsHR['activities-heart-intraday']['dataset']:
            print(f"no data for {day}")
            continue
        tmp = pd.DataFrame(fit_statsHR['activities-heart-intraday']['dataset']).rename({'value': day}, axis=1)
